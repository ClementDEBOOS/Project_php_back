...
$app->get('/actus', function (Request $request, Response $response, array $args) {
    $sth = $this->db->prepare("SELECT * FROM actualite ORDER BY date_ajout_actualite DESC LIMIT 5"); // Préparation de la requête
...
$app->delete('/suppActu/[{id}]', function ($request, $response, $args) {
    $sth = $this->db->prepare("DELETE FROM `actualite` WHERE `id_actualite`=:id"); // Préparation de la requête
    // Bindings de la requête
    $sth->bindParam("id", $args['id']);
    $sth->execute(); // Exécution de la requête
    return $this->response->withJson($input);
